...
const P2pFusionPanelContent: React.FC<P2pFusionPanelProps> = ({
  onStart,
  status,
  phase,
  busy,
    ...
    <div className="space-y-2.5 rounded-xl border border-violet-400/20 bg-violet-400/5 p-3">
      ...
      <div className="mt-2 rounded-lg border border-violet-400/20 bg-violet-400/5 px-2.5 py-2 text-[10px] wallet-muted leading-relaxed space-y-1">
        ...
        <ul className="list-disc pl-3.5 space-y-0.5">
          ...
          <li>
            <span className="wallet-text-strong">NIP-59 gift-wrap</span> —
            encrypt round messages on Nostr
          </li>
          <li>
          ...
          <li>
            <span className="wallet-text-strong">Output onion</span> — peers
            peel + shuffle outputs (not the same as Tor)
          </li>
        </ul>
        <p className="pt-0.5">
      ...
      <p className="text-[10px] leading-relaxed wallet-muted">
        No server: peers meet on Nostr over Tor, elect a coordinator, and run
        the CoinJoin peer-to-peer. Outputs use throwaway keys + mandatory
        peel-onion among peers. You sign only your own inputs after verifying
        your outputs — a hostile coordinator cannot make you sign away funds.
      </p>

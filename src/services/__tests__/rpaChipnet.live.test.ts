...
const enabled = process.env.RUN_RPA_LIVE === '1' && senderMnemonic.trim().length > 0;
...
describe.skipIf(!enabled)('RPA on chipnet', () => {
  it('pays a cashcode, finds it again by scanning, and can spend it', async () => {
    ...
    const recipient = await deriveRpaKeys(recipientMnemonic, '', Network.CHIPNET);
    const cashcode = await deriveAndEncodePaycode(recipientMnemonic, '', Network.CHIPNET);
    ...
    const dummyPrivkey = await derivePrivateKeyAtPath(senderMnemonic, '', dummyPath);
    ...
    const draft: TransactionCommon = {
      ...
      outputs: [
        { lockingBytecode: p2pkhLock(dummyPubkey), valueSatoshis: BigInt(paySats) },
        {
    ...
    const fetched = await anyServer([[1, 'blockchain.transaction.get', [txid, false]]]);
    ...
    console.log(`scanned     : ${matches[0].address} ${matches[0].valueSats} sats`);
    ...
    const spendPrivkey = await deriveSpendingKey(recipient.spendPrivkey, shared, 0);
    ...
    expect(binToHex(onChain.outputs[matches[0].outputIndex].lockingBytecode)).toBe(

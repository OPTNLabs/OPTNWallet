...
describe('shared RPA vectors', () => {
  it('matches the external bch-rpa reference values', () => {
    ...
    expect(
      binToHex(
        computeSharedSecret(smallKey(7), hexToBin(r.spendPrivkey13), txid, 0)
      )
    ...
    expect(
      binToHex(
        computeSharedSecret(smallKey(37), hexToBin(r.scanPrivkey7), txid, 0)
      )
  ...
  for (const w of vectors.wallets) {
    it(`derives the ${w.network} vectors from the shared mnemonic`, async () => {
      ...
      expect(rpaGrindString(keys.scanPubkey, RPA_PREFIX_BITS)).toBe(
        w.grindString16
      );

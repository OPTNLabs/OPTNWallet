...
describe('builtin addon visibility', () => {
  it('keeps dev-only apps visible in browser dev builds', async () => {
    ...
    const paryonApp = demo?.apps?.find(
      (app) => app.id === 'paryonWorkspaceApp'
    );
    ...
    expect(demo?.apps?.some((app) => app.id === 'paryonWorkspaceApp')).toBe(
      true
    );
    expect(
      demo?.contracts?.some(
        (contract) => contract.id === 'paryon-contract-bundle'
      )
    ).toBe(true);
    expect(fundme?.apps?.some((app) => app.id === 'fundmeApp')).toBe(true);
    expect(merchantApp).toBeDefined();
    expect(merchantApp?.devOnly).not.toBe(true);
  ...
  it('hides dev-only apps from packaged builds', async () => {
    ...
    expect(demo?.apps?.some((app) => app.id === 'merchantPayApp')).toBe(true);
  });
});

...
describe('appsViewHelpers', () => {
  it('keeps the existing category routing rules', () => {
    ...
    expect(getAppCategory({ id: 'optn.builtin.merchant-pay:merchantPayApp', name: 'Merchant Pay' })).toBe('Wallet');
  ...
  it('keeps the existing sort priorities and descriptions', () => {
    ...
    expect(getAppSortPriority({ id: 'optn.builtin.merchant-pay:merchantPayApp', name: 'Merchant Pay' })).toBe(1);
  ...
  it('keeps the existing icon-frame and hidden-app rules', () => {
    ...
    expect(getAppIconFrame({ name: 'Merchant Pay' })).toContain('bg-emerald-500/15');
    expect(shouldHideApp('optn.builtin.demo:authguard', 'AuthGuard')).toBe(true);
    expect(shouldHideApp('optn.builtin.merchant-pay:merchantPayApp', 'Merchant Pay')).toBe(true);

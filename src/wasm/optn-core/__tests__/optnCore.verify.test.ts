...
describe('optn-core through wasm', () => {
  ...
  it('matches the external bch-rpa reference secrets', () => {
    ...
    expect(
      bytesToHex(
        sharedSecret(smallKey(7), hexToBytes(r.spendPrivkey13), txid, 0)
      )
    ...
    expect(
      bytesToHex(
        sharedSecret(smallKey(37), hexToBytes(r.scanPrivkey7), txid, 0)
      )
  ...
  it('rejects a wrong-length key at the boundary instead of computing on it', () => {
    ...
    expect(() =>
      sharedSecret(
        new Uint8Array(31),
        hexToBytes(vectors.wallets[0].scanPubkey),
        'ab'.repeat(32),
        0
      )
    ).toThrow(/32 bytes/);
  ...
  for (const w of vectors.wallets) {
    it(`reproduces every ${w.network} vector`, () => {
      ...
      expect(paymentAddress(spend, secret, w.network, 0)).toBe(
        w.paymentAddress
      );
    });
  }

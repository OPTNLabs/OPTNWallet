...
if (!subtle) {
  console.error(
    '[WalletCrypto] Web Crypto API (crypto.subtle) is unavailable in this context.'
  );
}
...
export async function deriveKey(
  ...
  if (!subtle) {
    throw new Error(
      '[WalletCrypto] Web Crypto API is unavailable. Run the native Tauri desktop app instead.'
    );
  }
  const baseKey = await subtle.importKey(
    'raw',
    toArrayBufferBytes(enc.encode(passphrase)),
    { name: 'PBKDF2' },
    false,
    ['deriveKey']
  );
...
export async function aesEncrypt(
  ...
  const cipher = await subtle.encrypt(
    ...
    key,
    toArrayBufferBytes(enc.encode(plaintext))
  );

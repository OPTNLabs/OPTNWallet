...
/**
 * Switch the open wallet to another network WITHOUT a password re-prompt.
 *
 * A wallet is bound to one network at creation (its addresses derive under that
 * network's prefix), so "switch network" means moving to the SAME seed's twin
 * wallet on the target network. The twin is created with the source wallet's
 * exact in-RAM key and its salt, so the key already cached decrypts it — no
 * prompt, no lock-out. On a later switch back, the twin is found by matching
 * salt + decrypted mnemonic (only twins made this way share the key; a
 * separately-imported same-seed wallet keeps its own password and is skipped).
 *
 * Returns the twin's id + type, or null if there is no open wallet / no cached
 * key / a legacy (saltless) source — the caller then falls back to the picker.
 */
export async function switchNetworkSameSeed(
  sourceWalletId: number,
  targetNetwork: Network
): Promise<{ walletId: number; walletType: WalletType } | null> {
  const key = getCachedWalletKey();
  if (!key || sourceWalletId <= 0) return null;

  const salt = await readKdfSalt(sourceWalletId);
  const row = await readWalletRow(sourceWalletId);
  if (!salt || !row || !row.mnemonic.startsWith(SECRET_ENC_PREFIX)) return null;

  let mnemonic: string;
  let passphrase = '';
  try {
    mnemonic = await aesDecrypt(key, row.mnemonic.slice(SECRET_ENC_PREFIX.length));
    if (row.passphrase.startsWith(SECRET_ENC_PREFIX)) {
      passphrase = await aesDecrypt(key, row.passphrase.slice(SECRET_ENC_PREFIX.length));
    }
  } catch {
    return null; // cached key doesn't decrypt the open wallet (shouldn't happen)
  }

  const walletType =
    row.walletType === WalletType.QUANTUMROOT ? WalletType.QUANTUMROOT : WalletType.STANDARD;

  // Find an existing twin on the target network: same salt (so the cached key
  // opens it) AND same decrypted mnemonic.
  const manager = WalletManager();
  const sourceSaltB64 = bytesToBase64(salt);
  const all = (await manager.getAllWallets()) as Array<{ id: number; networkType: string }>;
  for (const w of all) {
    if (w.id === sourceWalletId || w.networkType !== targetNetwork) continue;
    const twinSalt = await readKdfSalt(w.id);
    if (!twinSalt || bytesToBase64(twinSalt) !== sourceSaltB64) continue;
    const twinRow = await readWalletRow(w.id);
    if (!twinRow?.mnemonic.startsWith(SECRET_ENC_PREFIX)) continue;
    try {
      const twinMnemonic = await aesDecrypt(key, twinRow.mnemonic.slice(SECRET_ENC_PREFIX.length));
      if (twinMnemonic === mnemonic) {
        return {
          walletId: w.id,
          walletType:
            twinRow.walletType === WalletType.QUANTUMROOT
              ? WalletType.QUANTUMROOT
              : WalletType.STANDARD,
        };
      }
    } catch {
      /* not this seed's twin */
    }
  }

  // No twin yet — create one on the target network. createWallet encrypts the
  // mnemonic/passphrase with the currently-cached key (the source's), and we
  // give the twin the source's salt so the same password/key reopens it.
  ...
  const twinId = await findNewestWalletIdByName(row.name);
  if (twinId == null) return null;
  await writeKdfSalt(twinId, salt);
  return { walletId: twinId, walletType };
}

...
pub fn build_and_attach(app: &tauri::App) -> tauri::Result<()> {
    ...
    let new_wallet = MenuItem::with_id(handle, "new_wallet", "New Wallet", true, Some("CmdOrCtrl+N"))?;
    let import_wallet =
        MenuItem::with_id(handle, "import_wallet", "Import Wallet", true, None::<&str>)?;
    let open_wallet =
        MenuItem::with_id(handle, "open_wallet", "Open Wallet…", true, Some("CmdOrCtrl+O"))?;
    let lock_wallet =
        MenuItem::with_id(handle, "lock_wallet", "Lock Wallet", true, Some("CmdOrCtrl+L"))?;
    ...
    let wallet_menu = Submenu::with_items(
        ...
        "Wallet",
        true,
        &[&receive, &send, &history, &PredefinedMenuItem::separator(handle)?, &settings],
    )?;

    let toggle_theme =
        MenuItem::with_id(handle, "toggle_theme", "Toggle Light / Dark", true, None::<&str>)?;
    ...
    let about =
        MenuItem::with_id(handle, "about", "About OPTN Wallet", true, None::<&str>)?;
    let help_menu = Submenu::with_items(handle, "Help", true, &[&about])?;

    let menu = Menu::with_items(
        handle,
        &[&file_menu, &wallet_menu, &view_menu, &tools_menu, &help_menu],
    )?;

...
| 4 | Tor modelled as transport policy, not a provider | **PARTIAL** | `Bip37Transport` / `NeutrinoTransport` / `TorStatus`; routing component checks | Three gaps. (a) `optn-chain-native::is_tor_socks_port` sends a SOCKS5 no-auth greeting and accepts `05 00`, which any SOCKS5 proxy offering no-auth satisfies -- so it proves SOCKS5, not Tor, while `optn-core/src/tor.rs` states the rule as "the port must have been probed and have answered as Tor" and warns about "a proxy that quietly forwards in the clear". `TorStatus::Unverified` exists for exactly this and the native runtime never produces it: a greeting goes straight to `Verified`. Fingerprinting Tor over SOCKS is not reliable, so the fix is provenance -- a port the app started and owns, or one the holder explicitly trusted -- which needs a durable trust setting this overlay does not have yet. (b) Leptos startup does not consume the existing Tor manager's readiness. (c) CLI needs the same transport choice |

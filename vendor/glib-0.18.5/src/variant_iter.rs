...
mod tests {
    ...
    #[test]
    fn test_variant_str_iter_output_pointer() {
        // RUSTSEC-2024-0429: run with --release to exercise the optimized FFI read.
        let v = ["first", "", "λ", "last"].to_variant();
        assert_eq!(v.array_iter_str().unwrap().next(), Some("first"));
        assert_eq!(v.array_iter_str().unwrap().nth(2), Some("λ"));
        assert_eq!(v.array_iter_str().unwrap().last(), Some("last"));
        assert_eq!(v.array_iter_str().unwrap().next_back(), Some("last"));
        assert_eq!(v.array_iter_str().unwrap().nth_back(2), Some(""));

...
import { appendSecretTreeValues, createSecretTree, secretTreeDecoder, secretTreeEncoder, } from "./secretTree.js";
...
import { isAppDataUpdateProposal, isDefaultProposal, isSelfRemoveProposal, } from "./proposal.js";
import { applyAppDataUpdates, appDataUpdateProposalType } from "./appDataUpdate.js";
...
import { bigintMapEncoder, bigintMapDecoder, varLenDataDecoder, varLenDataEncoder, varLenTypeDecoder, varLenTypeEncoder, } from "./codec/variableLength.js";
...
const treeHashCacheEncoder = varLenTypeEncoder(optionalEncoder(varLenDataEncoder));
const treeHashCacheDecoder = varLenTypeDecoder(optionalDecoder(varLenDataDecoder));
/** @public */
...
async function validateExternalSenders(extensions, authService) {
    const externalSenders = extensions.find((e) => e.extensionType === defaultExtensionTypes.external_senders);
    if (externalSenders) {
...
export async function validateRatchetTree(tree, groupContext, config, authService, treeHash, cs, mutableTreeHashCache) {
...
function extractAppDataUpdates(allProposals) {
...
function extractSelfRemoveLeafIndices(allProposals) {
    return allProposals.flatMap(({ proposal, senderLeafIndex }) => isSelfRemoveProposal(proposal) && senderLeafIndex !== undefined ? [senderLeafIndex] : []);
}
function validateSelfRemoveProposals(allProposals, committerLeafIndex, grouped, tree) {
    const seen = new Set();
    for (const { proposal, senderLeafIndex } of allProposals) {
        if (!isSelfRemoveProposal(proposal))
            continue;
        ...
        if (grouped[defaultProposalTypes.remove].some((r) => r.proposal.remove.removed === senderLeafIndex) ||
...
function validateAppDataUpdateProposals(allProposals, gceExtensions, currentExtensions) {
    ...
    if (gceExtensions !== undefined) {
        const requiresAppDataUpdate = (extensions) => extensions
            .find((e) => e.extensionType === defaultExtensionTypes.required_capabilities)
        ...
        if (requiresAppDataUpdate(currentExtensions) || requiresAppDataUpdate(gceExtensions)) {
            const currentDictionary = currentExtensions.find((e) => e.extensionType === appDataDictionaryExtensionType)?.extensionData;
            const proposedDictionary = gceExtensions.find((e) => e.extensionType === appDataDictionaryExtensionType)?.extensionData;
            const dictionaryUnchanged = currentDictionary === undefined
                ? proposedDictionary === undefined
                : proposedDictionary !== undefined &&
                    constantTimeEqual(currentDictionary, proposedDictionary);
            if (!dictionaryUnchanged)
                return new ValidationError("GroupContextExtensions proposal cannot modify the app_data_dictionary extension when required capabilities include the AppDataUpdate proposal type");
        }
    }
}
export async function applyProposals(state, mutableTree, proposals, committerLeafIndex, pskSearch, sentByClient, clientConfig, authService, cs) {
    ...
    if (!isExternalInit) {
        ...
        const updatedExtensions = appDataUpdates.length > 0
            ? applyAppDataUpdates(newExtensions ?? state.groupContext.extensions, appDataUpdates, clientConfig.appDataUpdateCallback)
            : newExtensions;
        const addedLeafNodes = await applyTreeMutations(mutableTree, grouped, state.groupContext, sentByClient, authService, clientConfig.lifetimeConfig, cs.signature, selfRemoveLeafIndices);
        ...
        const needsUpdatePath = allProposals.length === 0 ||
            allProposals.some(({ proposal }) => {
                ...
                return (t !== defaultProposalTypes.add &&
...
async function applyTreeMutations(mutableTree, grouped, gc, sentByClient, authService, lifetimeConfig, s, selfRemoveLeafIndices = []) {

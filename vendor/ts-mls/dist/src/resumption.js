...
export async function reinitCreateNewGroup(params) {
    const { context, state, keyPackage, privateKeyPackage, memberKeyPackages, groupId, cipherSuite, extensions, provider, ratchetTreeExtension, wireAsPublicMessage, } = params;
    const authService = context.authService;
...
export async function branchGroup(params) {
    const { context, state, keyPackage, privateKeyPackage, memberKeyPackages, newGroupId, ratchetTreeExtension, wireAsPublicMessage, } = params;
    const cs = context.cipherSuite;
    const authService = context.authService;

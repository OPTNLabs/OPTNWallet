...
impl Default for ProgressiveSyncConfig {
    fn default() -> Self {
        Self { header_start_height: 1, header_batch_size: 2_000, max_header_batches: 1_000 }
    }
}
...
pub enum ProgressiveSyncError {
    NoWalletRoute,
    MissingHeaderRoute { source: SourceId, protocol: ProtocolFamily },
    HeaderSafetyLimit,
...
impl ProgressiveSyncWorker {
    pub fn new(config: ProgressiveSyncConfig) -> Self {
        Self { config, reconciliation: ReconciliationState::default() }
    }
    ...
    pub async fn refresh(
        ...
        for route in routes {
            ...
            let request = ChainRequest::WalletRefresh { interests: interests.clone(), from_height };
            match service.execute_on_route(&route, &request).await {
                Ok(observation) => {
                    let ChainPayload::WalletRefresh { transactions, tip } = observation.value else {
                        self.reconciliation.record_failure("wallet route returned unexpected payload");
                        continue;
                    };
                ...
                Err(error) => {
                    self.reconciliation.record_failure(format!("wallet refresh failed: {error:?}"));
                }
            }
        }
    ...
    async fn prime_headers_on_same_route(
        ...
        let header_route = service.routes_for_operation(ChainOperation::HeaderSync)
            .into_iter()
            .find(|candidate| {
            ...
            .ok_or_else(|| ProgressiveSyncError::MissingHeaderRoute {
                source: wallet_route.source.clone(), protocol: wallet_route.protocol,
            })?;
...
mod tests {
    ...
    impl ChainBackend for WalletBackend {
        ...
        fn execute<'a>(&'a self,_:&'a ChainRequest)->ChainFuture<'a,BackendObservation>{Box::pin(async move{Ok(BackendObservation{payload:ChainPayload::WalletRefresh{transactions:vec![],tip:Some(ChainTip{height:7,hash:[7;32]})},evidence:Evidence::ServerAssertion,chain_tip:Some((7,[7;32]))})})}
    }

    #[tokio::test]
    async fn successful_refresh_reconciles_snapshot() {
        let id=SourceId::new("server"); let mut catalog=SourceCatalog::default();
        catalog.insert(ChainSource{id:id.clone(),label:"server".into(),origin:SourceOrigin::UserAdded,endpoints:vec![Endpoint{kind:EndpointKind::ElectrumTcp,host:"server".into(),port:Some(50001)}],capabilities:CapabilitySet::default(),disposition:SourceDisposition::Enabled,priority:0}).unwrap();
        let mut caps=CapabilitySet::default();caps.record(Capability::UtxoQuery,CapabilityConfidence::Verified,CapabilityDiscovery::ActiveProbe);
        let mut service=ChainService::new(catalog,ConnectionPolicy::auto());service.register(Arc::new(WalletBackend{id,caps}));
        ...
        assert_eq!(worker.reconciliation().authoritative.as_ref().unwrap().value.tip.as_ref().unwrap().height,7);
    }
}

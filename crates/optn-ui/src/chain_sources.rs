        ...
        <section aria-label="Chain sources">
            ...
            <Show when=move || page.get()==NetworkPage::Services>
                ...
                <form class="watch-only-form" on:submit=move |event| {
                    ...
                    <label class="field"><span>"Connection"</span><select prop:value=move || kind.get() on:change=move |event| kind.set(event_target_value(&event))><option value="electrum-tls">"Electrum TLS"</option><option value="electrum-tcp">"Electrum TCP"</option><option value="p2p">"BCH node"</option><option value="node-rpc">"Node RPC"</option><option value="node-zmq">"Node notifications"</option><option value="ipfs-gateway">"IPFS gateway (HTTPS / verified Tor)"</option></select></label>

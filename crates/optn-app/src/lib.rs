...
pub enum AppAction {
    ...
    /// Replace chain routes for one specified network after validating them.
    /// Explorer is preserved unless the payload includes one. Used by legacy
    /// renderer adapters that select one backend at a time.

...
mod tests {
    ...
    fn legacy_ciphertext_migrates_without_changing_identity_or_optional_metadata() {
        // Independent Node crypto fixture: published BIP39 phrase + TREZOR,
        // PBKDF2-SHA256/600000 and nonce || AES-256-GCM ciphertext || tag.
        // Fixed salt/nonces are only for this published, unfunded test vector.
        let file =
            WalletFile::parse(include_bytes!("../tests/fixtures/legacy-wallet-v1.json")).unwrap();
        let (_, account) = file.account(Network::Mainnet).unwrap();
        let expected = Wallet::from_mnemonic(crate::hd::BIP39_TEST_VECTOR_MNEMONIC, "TREZOR")
            .unwrap()
            .account_xpub_at(account)
            .unwrap();
        assert_eq!(
            file.unlock("old-password")
                .unwrap()
                .account_xpub_at(account)
                .unwrap(),
            expected
        ...
        let changed = file
            .change_password("old-password", "new-password", "new-password", &entropy)
            .unwrap();
        ...
        assert!(changed.unlock("old-password").is_err());
    }

    #[test]
    fn password_and_os_empty_value_rules_are_distinct() {
        assert!(validate_new_password("", "").is_ok());
        assert!(validate_new_password("1234567", "1234567").is_err());
        assert!(validate_new_password("12345678", "12345678").is_ok());
        assert!(validate_new_password("12345678", "different").is_err());

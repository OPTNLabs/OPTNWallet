...
pub trait FusionIo: AsyncRead + AsyncWrite + Unpin + Send {}
impl<T: AsyncRead + AsyncWrite + Unpin + Send> FusionIo for T {}
pub type FusionStream = Box<dyn FusionIo>;
...
pub const VERSION: &[u8] = b"alpha13";
...
pub const FUSION_EXECUTION_PAUSED_MESSAGE: &str =
...
pub const fn fusion_execution_ready() -> bool {
    true
}
...
pub async fn send_frame<S>(stream: &mut S, payload: &[u8]) -> Result<(), String>
where
...
pub async fn recv_frame<S>(stream: &mut S) -> Result<Vec<u8>, String>
where
...
pub async fn recv_frame_with_timeout<S>(stream: &mut S, wait: Duration) -> Result<Vec<u8>, String>
where
    S: AsyncReadExt + Unpin,
{
...
pub async fn recv_frame_unbounded<S>(stream: &mut S) -> Result<Vec<u8>, String>
where
    S: AsyncReadExt + Unpin,
{
